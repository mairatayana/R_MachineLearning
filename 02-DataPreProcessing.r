...
iris6 <- iris[sample(nrow(iris),0.7*nrow(iris),replace = TRUE),]
myhist(iris6)
summary(iris6)
########################################################################################################
# Unbalanced data
...
#resultado da tabela positivo e negativo(F=Falso, V=Verdadeiro, N=Negativo, P=Positivo)
#    +    -
# +  VP   FP
# -  FN   VN
# 1nn with iris3
...
irisM$Petal.Length[inds] <- NA
View(irisM)
irisM$Petal.Length[is.na(irisM$Petal.Length)] <- mean(irisM$Petal.Length[which(irisM$Species=="virginica")], na.rm = TRUE)
irisM$Petal.Length[is.na(irisM$Petal.Length)] <- mean(irisM$Petal.Length[which(irisM$Species=="setosa")], na.rm = TRUE)

irisSetosa <- subset(iris,Species=="setosa")
inds <- sample(1:50, 1, replace=F)
irisSetosa[inds,]
irisSetosa$Petal.Length[inds] <- NA
View(irisSetosa)
irisSetosa$Petal.Length[is.na(irisSetosa$Petal.Length)] <- mean(irisSetosa$Petal.Length, na.rm = TRUE)
irisSetosa[inds,]

irisVersicolor <- subset(iris,Species=="versicolor")
inds <- sample(1:50, 1, replace=F)
irisVersicolor[inds,]
irisVersicolor$Petal.Length[inds] <- NA
View(irisVersicolor)
irisVersicolor$Petal.Length[is.na(irisVersicolor$Petal.Length)] <- mean(irisVersicolor$Petal.Length, na.rm = TRUE)
irisVersicolor[inds,]

irisVirginica <- subset(iris,Species=="virginica")
inds <- sample(1:50, 1, replace=F)
irisVirginica[inds,]
irisVirginica$Petal.Length[inds] <- NA
View(irisVirginica)
irisVirginica$Petal.Length[is.na(irisVirginica$Petal.Length)] <- mean(irisVersicolor$Petal.Length, na.rm = TRUE)
irisVirginica[inds,]
...
out <- ENN(Species~., data = irisPart, k = 1)
summary(out)
...
ir.pca <- prcomp(log.ir,
                 center = TRUE, # vaz media zero e varianca 1
                 scale. = TRUE) # vaz media zero e varianca 1
...
knnPCA <- knn.cv(pca[,1:2], pca$Species, k = 3)
tab <- table(knnPCA, pca$Species)
tab

knnTotal <- knn.cv(iris[,1:2], iris$Species, k = 3)
tab <- table(knn, iris$Species)
tab
