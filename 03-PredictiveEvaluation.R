...
#knn1 == irisPart$Species -> para ver o que deu certo e o que deu errado
...
# dividing the dataset into training and testing sets (80% para treinamento, 20% para teste)
...
linear <- lm(trainingData[,2]~trainingData[,1]) #regressor linear "linear model" para prever
pred <- predict(linear, data = testData[,1])

# mse
mse <- mean((testData[,2]-pred)^2) #erro médio quadrático 
mse

# Exercice: compute normalized mean squared error
nmse <- sum((testData[,2]-pred)^2)/sum((testData[,2]-mean(testData[,2]))^2)
nmse
...
set.seed(200)  # setting seed to reproduce results of random sampling
trainingRowIndex <- sample(1:nrow(iris), 0.8*nrow(iris))  # row indices for training data
trainingData <- iris[trainingRowIndex, ]  # model training data
summary(trainingData)
testData  <- iris[-trainingRowIndex, ] 
